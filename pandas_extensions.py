...
from .data_analysis import dist, string_format
...
def time_series_distribution(
    sample, sample_name=None, sample_size=300, freq="D"
):
...
def numeric_distribution(sample, sample_name=None, sample_size=50):
    ...
    if nb_unique <= 20:
        fig = text_distribution(sample.astype(str), sample_name=sample_name,)
    else:
    ...
    if sample_distribution.shape[0] > sample_size:
        ...
        if format_distribution.shape[0] > sample_size:
            ...
            fig = numeric_distribution(
                lengths, sample_name=f"Lenght of {sample_name}"
            )
        else:
            fig = text_distribution(
                formats, sample_name=f"Format of {sample_name}"
            )
    else:
        fig = go.Figure(
            data=[
                go.Bar(
                    x=sample_distribution.value, y=sample_distribution.freq,
                )
            ]
        )
        if sample_name is not None:
    ...
    fig.update_layout(
        annotations=[
            go.layout.Annotation(
                ...
                y=1.2,
                showarrow=False,
                text=f"Number of values: {self.shape[0]}"
                + "<br>"
                + f"Filled: {self.dropna().shape[0]}"
                + "<br>"
